...
{
    [Header("Prefabs")]
    [SerializeField] private NBodyPrefabs prefabs;

    [Header("Parameters")]
    [SerializeField, Min(1)] private int numBodies = 10;
    [SerializeField, Min(0)] private float mass = 1f;
    [SerializeField, Min(0.00001f), Tooltip("Softening length")] float epsilon = 0.1f;
    [SerializeField, Min(0.01f)] private float bodyRadius = 1;
    [SerializeField] private Vector3 systemPosition;
    [SerializeField, Min(0)] private float radialMean = 5;
    [SerializeField, Min(0)] private float radialSigma = 0;
    ...
    private void Awake()
    {
        ...
        double newtonG = Units.NewtonG(unitTime, unitLength, unitMass);

        // Create the N-Body solver
        solver = new NBodySolver(numBodies, newtonG, mass, epsilon);
        solver.Initialize(radialMean, radialSigma, systemPosition);

        // Draw the objects
        if (prefabs)
        {
            prefabs.CreateBodies(numBodies);
            for (int i = 0; i < solver.numBodies; i++)
            {
                Vector3 position = new Vector3((float)solver.x[i * 6 + 0], (float)solver.x[i * 6 + 1], (float)solver.x[i * 6 + 2]);
                prefabs.bodies[i].position = position;
                prefabs.bodies[i].localScale = 2 * bodyRadius * Vector3.one;
            }
        }
    }
